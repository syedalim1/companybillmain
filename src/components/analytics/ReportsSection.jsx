...
import PrintableSummaryModal from './PrintableSummaryModal';

const ReportsSection = ({ analytics, filters }) => {
  ...
  // Standard Header-Level Export Data
  ...
  // Item-Level Flattened Export Data (One row per line item)
  const buildFlattenedItemExportData = () => {
    const rows = [];
    filteredInvoices.forEach(inv => {
      ...
      const buyerName = inv.buyerName || inv.buyer?.name || '';
      ...
      (inv.items || []).forEach((item, idx) => {
        rows.push({
          ...
          'HSN/SAC': item.hsn || item.sac || '',
          'Quantity': safeNum(item.quantity),
          'Unit': item.unit || 'units',
          'Rate': safeNum(item.rate).toFixed(2),
          'Discount %': safeNum(item.discount).toFixed(2),
          'Line Taxable Amount': (safeNum(item.quantity) * safeNum(item.rate) * (1 - safeNum(item.discount) / 100)).toFixed(2),
        });
      });
    });
    return rows;
  ...
  const handleExport = () => {
    const data = exportFormat === 'item_csv' ? buildFlattenedItemExportData() : buildExportData();
    if (data.length === 0) { alert('No records available to export with current filters.'); return; }

    const dateSuffix = new Date().toISOString().slice(0, 10);

    if (exportFormat === 'csv' || exportFormat === 'item_csv') {
      const headers = Object.keys(data[0]);
      const csv = [
        headers.join(','),
        ...data.map(row => headers.map(h => `"${String(row[h] || '').replace(/"/g, '""')}"`).join(','))
      ].join('\n');
      const filename = exportFormat === 'item_csv' ? `flattened_line_items_${dateSuffix}.csv` : `invoice_summary_${dateSuffix}.csv`;
      downloadFile(csv, filename, 'text/csv');
    } else {
      downloadFile(JSON.stringify(data, null, 2), `analytics_export_${dateSuffix}.json`, 'application/json');
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Export & Printable Summary Controls */}
      <div className="bg-bg-surface rounded-2xl border border-slate-100 shadow-sm p-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h4 className="text-base font-bold text-text-title">Export & Management Reports</h4>
            <p className="text-xs text-text-desc mt-0.5">{filteredInvoices.length} matching document records</p>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl text-xs font-bold shadow-md hover:from-purple-500 hover:to-indigo-500 transition-all flex items-center gap-1.5"
            >
            ...
            >
              <option value="csv">CSV (Invoice Header Summary)</option>
              <option value="item_csv">CSV (Flattened Line Items)</option>
              <option value="json">JSON Format</option>
            </select>

            <button
              onClick={handleExport}
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-slate-800 to-slate-900 text-white rounded-xl hover:from-slate-700 hover:to-slate-800 shadow-md transition-all text-xs font-bold"
            >
      ...
      <DataTable
        ...
        subtitle={`Period: ${analytics.currentPeriodLabel}${filters.docType !== 'all' ? ` • ${filters.docType}` : ''}`}
        maxRows={25}
      />

      {/* Printable Management Summary Modal */}
