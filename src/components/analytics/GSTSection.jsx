...
const GSTSection = ({ analytics }) => {
  ...
  const { cgst, sgst, igst, total, taxable, intraCount, interCount, invoiceCount } = gstBreakdown;

  // Donut chart
  const size = 180; const sw = 24; const r = (size - sw) / 2; const circ = 2 * Math.PI * r;
  const segments = [
  ...
  // Monthly GST chart
  const maxMonthGST = Math.max(...monthlyGST.map(m => m.cgst + m.sgst + m.igst), 1);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          { label: 'IGST', value: formatINR(igst) },
          { label: 'Total GST', value: formatINR(total) },
        ].map((m, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-bg-surface rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col items-center">
          <h4 className="text-base font-bold text-text-title mb-1 self-start">Tax Split</h4>
          <p className="text-xs text-text-desc mb-4 self-start">{invoiceCount} GST invoices</p>
          {total === 0 ? (
              ...
              <div className="w-full space-y-2">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-text-title">{formatINR(item.amount)} <span className="text-text-desc">({item.pct.toFixed(1)}%)</span></span>
                  </div>
                ))}
              </div>
              <div className="w-full mt-4 pt-3 border-t border-slate-100 flex justify-between text-[10px]">
                <span className="text-text-desc">Intra-State: <span className="font-bold text-text-title">{intraCount}</span></span>
                <span className="text-text-desc">Inter-State: <span className="font-bold text-text-title">{interCount}</span></span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="lg:col-span-2 bg-bg-surface rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <p className="text-xs text-text-desc mb-4">CGST + SGST + IGST over 12 months</p>
          <div className="flex items-end gap-1 h-44">
            {monthlyGST.map((m, i) => {
              const totalM = m.cgst + m.sgst + m.igst;
